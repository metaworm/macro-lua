...
use libc::{c_void, c_int, c_long, c_char, c_uchar, size_t};
use std::ptr;

pub use crate::luaconf::{
    LUA_EXTRASPACE, LUA_NUMBER, LUA_INTEGER,
    LUA_REGISTRYINDEX, LUA_UNSIGNED, LUA_IDSIZE,
    LUA_FILEHANDLE, LUA_KCONTEXT, LUA_VERSION_NUM,
    LUAL_BUFFERSIZE, LUAL_NUMSIZES,
};
...
pub unsafe fn lua_getextraspace(L: *mut lua_State) -> *mut c_void {
    L.offset(-LUA_EXTRASPACE as isize) as *mut c_void
}
